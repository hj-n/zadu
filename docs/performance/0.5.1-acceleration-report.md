...
Measurements in this report were collected on 2026-08-23 for candidate
revision `94e71a9`. They do not measure the current checkout; later kernels,
memory plans, and dependency versions can change performance.

## Results at 2,000 samples

On the Apple M4 test machine, the common eight-metric suite measured:
...
Each speedup is baseline time divided by candidate time. Values above 1 are
faster. The final columns show the candidate's absolute time and peak RSS.

### Core rank/neighbor suite

| n | v0.5 cold | v0.5 warm | 2023 cold | 2023 warm | candidate cold | candidate warm | candidate RSS |
| --: | --: | --: | --: | --: | --: | --: | --: |
...
The 2023 comparison is not like-for-like: the candidate score differs by as much
...
At 2,000 samples the candidate/v0.5.0 maximum score delta is `6.11e-15`.
...
5.921 s warm versus 3.651 s for the candidate (**1.62x**), and cold collection time fell
...
machine. MLX had the lowest warm time in this table. Torch CPU preserves float64
...
The candidate kernels were compared with slow reference formulas on the
...
These timings exclude resource construction. The representative end-to-end
comparison measured a 1.64x default speedup for its specified workload.
...
These optimizations retained the algorithms and configured sample counts.
They did not introduce approximate neighbor selection; S&C retained its
randomized walks.

## Reproduction

Install the legacy dependencies from `benchmarks/requirements-history.txt`.
Create worktrees for the baselines and the measured candidate, then run the
benchmark from the development checkout:

```bash
git worktree add --detach /tmp/zadu-v0.1.1 v0.1.1
git worktree add --detach /tmp/zadu-v0.5.0 v0.5.0
git worktree add --detach /tmp/zadu-0.5.1-candidate 94e71a9
...
python benchmarks/benchmark_release_history.py \
  ...
  --source current=/tmp/zadu-0.5.1-candidate \
...
The raw records contain the environment used for these measurements. Match
those dependency versions to reproduce that environment; running the command
with newer dependencies is a new comparison.
