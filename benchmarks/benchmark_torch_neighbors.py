"""Compare NumPy and optional PyTorch exact neighbor resources."""
...
def main() -> None:
    ...
    torch_arrays = _value_arrays(torch_result)
    exact_match = all(
        np.array_equal(numpy_array, torch_array)
        for numpy_array, torch_array in zip(
            numpy_arrays,
            torch_arrays,
            strict=True,
        )
    ...
    mismatch_count = sum(
        int(np.count_nonzero(numpy_array != torch_array))
        for numpy_array, torch_array in zip(
            numpy_arrays,
            torch_arrays,
            strict=True,
