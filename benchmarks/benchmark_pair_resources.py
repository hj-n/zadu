"""Benchmark fused exact pair statistics against the legacy dense path."""
...
def _worker(args):
    ...
    if args.worker == "dense":
        ...
        ZADU(PAIR_SPECS, orig[:20], execution=execution).measure(emb[:20])
        cold_seconds, scores = _median(
            lambda: ZADU(PAIR_SPECS, orig, execution=execution).measure(emb),
            args.repeat,
        )
        runner = ZADU(PAIR_SPECS, orig, execution=execution)
...
def _run_worker(args, mode):
    ...
    if args.memory_budget is not None:
        command.extend(("--memory-budget", args.memory_budget))
