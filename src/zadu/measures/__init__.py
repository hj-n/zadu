__all__ = [
    'class_aware_trustworthiness_continuity', 'spearman_rho', 'neighborhood_dissimilarity', 
		'distance_to_measure', 'local_continuity_meta_criteria', 'internal_validation_measure', 
		'pearson_r', 'distance_consistency', 'kl_divergence', 'neighborhood_hit', 
		'trustworthiness_continuity', 'clustering_and_external_validation_measure', 'mean_relative_rank_error'	
]

## add to __all__ if the function is added