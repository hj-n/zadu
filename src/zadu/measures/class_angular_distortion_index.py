...
def _get_cluster_info(labels):
    ...
    if n_clusters < 2:
        raise ValueError("CADI needs at least 2 clusters to sample angles between clusters.")
    ...
    if not len(clusterB_idxs):
        raise ValueError("At least one cluster must have size >= 2 to sample y,z in triplet (x,y,z).")
