...
from zadu.engine.workspace import procrustes_workspace

from .utils import knn
from .utils.validation import validate_pair
from .utils.vectorized import iter_row_blocks
...
def measure(
    ...
    workspace = procrustes_workspace(
        points_num, orig_dim, emb_dim, neighbor_count, np.dtype(work_dtype).itemsize
    )
    block_budget = workspace.block_bytes(working_memory_bytes)
    bytes_per_row = workspace.bytes_per_row
    ...
    for block in iter_row_blocks(
        ...
        normalized_scores[block] = _block_scores(
            orig, emb, orig_knn_indices[block], emb_knn_indices[block], work_dtype
        )
...
def _block_scores(orig, emb, orig_indices, emb_indices, work_dtype):
    origin_neighbors = np.asarray(orig[orig_indices], dtype=work_dtype)
    embedded_neighbors = np.asarray(emb[emb_indices], dtype=work_dtype)
    centered_orig = origin_neighbors - np.mean(origin_neighbors, axis=1, keepdims=True)
